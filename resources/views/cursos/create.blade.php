...
@section('content')
    <section class="cursos-form-page">
        <form class="cursos-form-card" action="{{ route('cursos.store') }}" method="POST">
            @csrf
            ...
            <div class="cursos-form-grid">
                ...
                <div class="cursos-field">
                    ...
                    <input id="nombre" name="nombre" type="text" value="{{ old('nombre') }}" placeholder="Ej. PRL Básico, PEMP, Alturas">
                    @error('nombre') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="cursos-field">
                    <label for="descripcion">Descripción</label>
                    <textarea id="descripcion" name="descripcion" rows="5" placeholder="Detalla el contenido, validez o requisitos del curso">{{ old('descripcion') }}</textarea>
                ...
                <div class="cursos-form-grid" style="grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px;">
                    <div class="cursos-field">
                        ...
                        <input id="meses_validez" name="meses_validez" type="number" min="1" max="120" value="{{ old('meses_validez') }}" placeholder="Opcional">
                        @error('meses_validez') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>

                    <div class="cursos-field">
                        <label for="dias_aviso_previo">Días de aviso previo</label>
                        <input id="dias_aviso_previo" name="dias_aviso_previo" type="number" min="1" max="365" value="{{ old('dias_aviso_previo', 30) }}">
            ...
            <div class="cursos-actions">
                ...
                <button type="submit" class="cursos-btn cursos-btn--primary">Crear Curso</button>
            </div>
        </form>
    </section>
@endsection